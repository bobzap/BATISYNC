/*
  # Update extracted_vouchers view to include price unit

  1. Changes
    - Update the extracted_vouchers view to properly extract price unit from all voucher types
    - Add support for calculating total amount based on quantity and price unit
    - Improve the get_extracted_vouchers_by_project function with better filtering
    - Add a function to get vouchers with their invoice information
...
DROP FUNCTION IF EXISTS get_vouchers_with_invoices CASCADE;
...
BEGIN
  ...
  WHERE 
    (p_project_id IS NULL OR v.project_id = p_project_id)
...
BEGIN
  ...
  SELECT 
    ...
    NULL::validation_status AS invoice_status
  FROM 
    extracted_vouchers v
  WHERE 
