/*
  # Create view for extracting vouchers from daily reports
  
  1. New View
    - `extracted_vouchers` - Extracts voucher data from daily_reports jsonb columns
    - Combines bons_apport, bons_evacuation, bons_beton, and bons_materiaux
    - Maps to the voucher structure expected by the frontend
    
  2. Functions
    - Function to extract vouchers by project and date range
    
  3. Security
    - No RLS needed as this is just a view over existing tables
*/

-- Create a view to extract vouchers from daily reports
...
-- Create a function to get vouchers by project and date range
