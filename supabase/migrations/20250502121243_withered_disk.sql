/*
  # Ajout des tables pour l'extraction de données de contrats
  
  1. Nouvelles Tables
    - `contract_extracted_data` - Stocke les données extraites des contrats
    - `contract_data_rows` - Stocke les lignes de données extraites
    
  2. Relations
    - Données extraites liées aux contrats
    - Données extraites liées aux documents (optionnel)
