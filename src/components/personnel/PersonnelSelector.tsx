...
import { BasePersonnel, PersonnelFonction, ProjectPersonnel } from '../../types';
            ...
            <div className="grid grid-cols-1 gap-2 p-4">
              {loading ? (
                ...
                filteredPersonnel.map((projet) => {
                  if (!projet.personnel) return null;
                  const isSelected = selectedIds.includes(projet.personnel_id);
                  const fonction = fonctions.find(f => f.code === projet.personnel.intitule_fonction);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-lg font-medium ${
                        ...
                        {projet.personnel.nom.charAt(0).toUpperCase()}
                      </div>
                      ...
                      <div className="flex-grow text-left">
                        <div className="flex items-center gap-2">
                          <span className={`font-medium ${
                            ...
                            {projet.personnel.nom}
                          </span>
                          {projet.personnel.prenom && (
                            <span className={`font-medium ${
                              isDark ? 'text-gray-300' : 'text-gray-700'
                            }`}>
                              {projet.personnel.prenom}
                          ...
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-0.5 text-xs rounded-full ${
                              ...
                              {fonctions.find(f => f.code === projet.personnel.intitule_fonction)?.libelle || projet.personnel.intitule_fonction}
                            </span>
                            <span className={`px-2 py-0.5 text-xs rounded-full ${
                              isDark
                              ...
                              {projet.personnel.entreprise}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>
                           {projet.personnel.numero_personnel}
                          </span>
                          <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>
